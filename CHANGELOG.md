...
### 🐛 历史记录保存稳定性修复 + View Details 收藏功能

#### Bug 修复

1. **历史记录保存失败后 History 页面无限 Loading**
   - 根本原因一：`saveExamResult()` 无超时机制，网络抖动时 Supabase insert 请求永久挂起，`showHistoryScreen()` 中 `await state.pendingSave` 随之永久阻塞，页面停留在 "Loading history..."
   - 根本原因二：`saveExamResult()` 失败只有 `console.error`，用户在结果页面完全看不到任何提示，不知道保存已失败
   - 根本原因三：`showHistoryScreen()` 将网络错误与空记录混为一谈（`if (error || !data.length === 0)`），报错时显示 "No exams yet" 而非错误提示
   - 修复：见技术说明

2. **View Details 缺少收藏按钮**
   - 历史详情页（View Details）的逐题明细缺少 ☆ Favorite 按钮
   - 修复：补充与 Answer Review 页一致的收藏按钮及事件绑定

#### 新功能

- **结果页保存状态指示器**：提交考试后结果页实时显示保存进度
  - `Saving result to history...`（保存中）
  - `✓ Result saved to history`（保存成功，绿色）
  - `⚠ Failed to save result. Retry`（保存失败，红色，含 Retry 链接）
- **Retry 重试**：保存失败时可在结果页点击 "Retry" 重新保存，无需重做考试

#### 技术说明

- `saveExamResult()`：新增 10 秒超时（`Promise.race`），超时或报错时返回 `false`
- `submitTest()`：将两个保存操作打包为 `state.pendingSave = Promise.all([...])`，并通过 `.then()/.catch()` 更新结果页保存状态 UI；将 `state.lastResults` 保存供重试使用
- `showHistoryScreen()`：
  - 等待 `state.pendingSave` 时加 12 秒超时兜底（`Promise.race`），彻底防止无限 loading
  - 拆分错误处理：网络错误显示红色提示，空记录才显示 "No exams yet"
- 新增 `retrySave()` 函数：复用 `state.lastResults` 重新调用 `saveExamResult()`
- `state` 新增 `pendingSave`、`lastResults` 字段
- `index.html`：结果页新增 `<p id="save-status">` 元素
- `styles.css`：新增 `.save-status-msg`、`.saving`、`.save-ok`、`.save-fail` 样式
